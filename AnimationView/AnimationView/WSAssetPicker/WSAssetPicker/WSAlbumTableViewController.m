...
- (void)viewDidLoad
{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:WSTableNavBgView]] forBarMetrics:UIBarMetricsDefault];
    ...
    [self.assetsLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
        ...
        if (group == nil) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.navigationItem.title = AlbumStr;
            });
            return;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIImageView *leftImage = [[UIImageView alloc]initWithFrame:CGRectMake(7, 5, 50, 50)];
    [leftImage setImage:[UIImage imageWithCGImage:[group posterImage]]];
    [cell addSubview:leftImage];
    NSString *titleStr = [NSString stringWithFormat:@"%@", [group valueForProperty:ALAssetsGroupPropertyName]];
    CGFloat widTh = [titleStr sizeWithFont:[UIFont boldSystemFontOfSize:14]].width;
    UILabel *titleL = [[UILabel alloc]initWithFrame:CGRectMake(67, 30 - 15/2, widTh, 15)];
    [titleL setBackgroundColor:[UIColor clearColor]];
    [titleL setFont:[UIFont boldSystemFontOfSize:14]];
    [titleL setTextColor:[UIColor whiteColor]];
    [titleL setText:titleStr];
    [cell addSubview:titleL];
    
     NSString *titleStr2 = [NSString stringWithFormat:@"(%d)", [group numberOfAssets]];
    CGFloat widTh2 = [titleStr sizeWithFont:[UIFont boldSystemFontOfSize:14]].width;

    UILabel *titleL2 = [[UILabel alloc]initWithFrame:CGRectMake(titleL.frame.origin.x + titleL.frame.size.width + 8, 30 - 13/2, widTh2, 13)];
    [titleL2 setBackgroundColor:[UIColor clearColor]];
    [titleL2 setFont:[UIFont systemFontOfSize:12]];
    [titleL2 setTextColor:[UIColor colorWithRed:181.0f/255.0f green:181.0f/255.0f blue:181.0f/255.0f alpha:1.0f]];
    [titleL2 setText:titleStr2];
    [cell addSubview:titleL2];
    // Setup the cell.
...
//    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    UIImageView *imageView = [[UIImageView alloc]initWithImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:WStableCellSeqLine]]];
    [imageView setFrame:CGRectMake(0, ROW_HEIGHT - 1, 320, 2)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WSAssetTableViewController *assetTableViewController = [[WSAssetTableViewController alloc] init];
