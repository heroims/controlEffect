...
- (void)setCellAssetViews:(NSArray *)assets
{
    ...
    for (WSAssetWrapper *assetWrapper in assets) {
        ...
        if ([[assetWrapper.asset valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
            duration=[assetWrapper.asset valueForProperty:ALAssetPropertyDuration];
        }
        WSAssetViewColumn *assetViewColumn = [[WSAssetViewColumn alloc] initWithImage:[UIImage imageWithCGImage:assetWrapper.asset.thumbnail] duration:duration];
        assetViewColumn.column = [assets indexOfObject:assetWrapper];
        assetViewColumn.selected = assetWrapper.isSelected;
        assetViewColumn.assetPickerState=((WSAssetTableViewController*)_delegate).assetPickerState;
